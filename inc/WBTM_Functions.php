    ...
    if ($post_id && $start_route && $end_route) {
        ...
        if (sizeof($price_infos) > 0) {
            foreach ($price_infos as $price_info) {
                // Only process the prices for the selected start and end routes
                if (strtolower($price_info['wbtm_bus_bp_price_stop']) == strtolower($start_route) &&
                    strtolower($price_info['wbtm_bus_dp_price_stop']) == strtolower($end_route)) {

                    $adult_price  = array_key_exists('wbtm_bus_price', $price_info) && $price_info['wbtm_bus_price'] ? (float) $price_info['wbtm_bus_price'] : null;
                    $child_price  = array_key_exists('wbtm_bus_child_price', $price_info) && $price_info['wbtm_bus_child_price'] ? (float) $price_info['wbtm_bus_child_price'] : null;
                    $infant_price = array_key_exists('wbtm_bus_infant_price', $price_info) && $price_info['wbtm_bus_infant_price'] ? (float) $price_info['wbtm_bus_infant_price'] : null;

                    // Process adult price if available
                    if (!is_null($adult_price)) {
                        $ticket_infos[] = [
                    ...
                    // Process child price if available
                    if (!is_null($child_price)) {
                        $ticket_infos[] = [
                    ...
                    // Process infant price if available
                    if (!is_null($infant_price)) {
                        $ticket_infos[] = [
...
// 		if (sizeof($price_infos) > 0) {
// 			foreach ($price_infos as $price_info) {
// 				// Check for each price individually
// 				$adult_price  = array_key_exists('wbtm_bus_price', $price_info) && $price_info['wbtm_bus_price'] ? (float) $price_info['wbtm_bus_price'] : null;
// 				$child_price  = array_key_exists('wbtm_bus_child_price', $price_info) && $price_info['wbtm_bus_child_price'] ? (float) $price_info['wbtm_bus_child_price'] : null;
// 				$infant_price = array_key_exists('wbtm_bus_infant_price', $price_info) && $price_info['wbtm_bus_infant_price'] ? (float) $price_info['wbtm_bus_infant_price'] : null;
...
// 												// Updated condition to remove required validation for adult_price
// 												if ( isset( $adult_price ) && (float) $adult_price >= 0 ) {
// 														$ticket_infos[] = [
// 																'name'  => WBTM_Translations::text_adult(),
// 																'price' => MP_Global_Function::get_wc_raw_price( $post_id, $adult_price ),
// 																'type'  => 0
...
// 			}
			...
			//==========================//
			public static function get_seat_price( $post_id, $start_route, $end_route, $seat_type = 1, $dd = false ) {
				if ( $post_id && $start_route && $end_route ) {
